...
|---|---|---|
| Bundle | Top-level Sigstore bundle with OMS constraints | [`schemas/v1.0/bundle.schema.json`](../schemas/v1.0/bundle.schema.json) |
| Envelope | DSSE envelope (payloadType, signatures) | [`schemas/v1.0/envelope.schema.json`](../schemas/v1.0/envelope.schema.json) |
| Statement | in-toto Statement with OMS predicateType | [`schemas/v1.0/statement.schema.json`](../schemas/v1.0/statement.schema.json) |
| Predicate | OMS predicate body (resources, serialization) | [`schemas/v1.0/predicate.schema.json`](../schemas/v1.0/predicate.schema.json) |
...
> [!WARNING]
> **Future deprecation:** Support for `allow_symlinks: true` is expected
> to be removed in a future version of this specification. New signers
> are encouraged to set `allow_symlinks` to `false`.
...
**Example serialization object (using the default shard size):**

```json
{
  "method": "shards",
  "hash_type": "sha256",
  "shard_size": 1000000000,
  "allow_symlinks": false
}
```

**Example:** A 2,500,000,000-byte (2.5 GB) file `weights.bin` with the
default shard size of 1,000,000,000 produces three resource descriptors:

```json
{ "name": "weights.bin:0:1000000000",          "algorithm": "sha256", "digest": "a1b2..." },
{ "name": "weights.bin:1000000000:2000000000", "algorithm": "sha256", "digest": "c3d4..." },
{ "name": "weights.bin:2000000000:2500000000", "algorithm": "sha256", "digest": "e5f6..." }
```

The first two ranges each cover 1,000,000,000 bytes; the third covers
the remaining 500,000,000 bytes.
...
schema ([`schemas/v1.0/bundle.schema.json`](../schemas/v1.0/bundle.schema.json))
...
[`schemas/v1.0/statement.schema.json`](../schemas/v1.0/statement.schema.json):
...
JSON Schemas in `schemas/v1.0/` validate the **current** format.  Verifiers
...
The schemas in `schemas/v1.0/` validate **only** the current format.
